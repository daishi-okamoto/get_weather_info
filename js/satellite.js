$(document).ready(function() {
    ...
    function resetSatelliteImage() {
        ...
        const path = 'images/satellite/' + area + '/' + year + '/' + month + '/' + day + '/' +
            'satellite_' + area + '_' + year + month + day + '_' + hour + '_30_00.jpg';

        console.log(path);

        $('#SatelliteImage').append('<div></div>');
        $('#SatelliteImage > div').last().css('animation-duration', delay + 'ms');
        $('#SatelliteImage > div').last().css('background-image', 'url(' + path + ')');
        $('#SatelliteImage > div').first().delay(delay).queue(function() {
            $(this).remove();
        })
    }
